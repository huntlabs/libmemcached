...
*/
	...
	*/
	memcached_return_t fetch( string key,
							 out byte[] ret_val,
							  out uint flags,
							  out ulong cas_value)
	{
	...
	*/
	T get(T=string)(string key)
	{
		...
		char * value = memcached_get(memc_,cast(const char *)key.ptr, key.length,
	...
	* @return true on success; false otherwise
	*/
	T getByKey(T=string)( string master_key,
				   string key)
	{
		import std.c.stdlib;
		uint flags= 0;
	...
	*/
			 ...
			 uint flags = 0)
	{
		...
		memcached_return_t rc = memcached_set(memc_,
											 ...
											 cast(const char *)mvalue.ptr, mvalue.length,
											 cast(time_t)expiration, flags);
...
*/
	...
	* @return true on succcess; false otherwise
	*/
	bool setByKey(T=string)( string master_key,
				   string  key,
				   T value,
				   int expiration = 0,
				   uint flags = 0)
	{
		...
		return memcached_success(memcached_set_by_key(memc_, cast(const char *)master_key.ptr,
													  master_key.length,
													  cast(const char *)key.ptr, key.length,
													  cast(const char *)mvalue.ptr, mvalue.length,
													  cast(time_t)expiration,
													  flags));
	}
...
*/
	...
	*/
	...
	{
		return memcached_success(memcached_increment(memc_, cast(const char *)key.ptr, key.length, offset, value));
	}
	...
	*/
	...
	{
		return memcached_success(memcached_decrement(memc_, cast(const char *)key.ptr,
													 key.length,
													 offset, value));
	}
	...
	*/
	bool add(T=string)(string key, T tvalue)
	{
		...
		return memcached_success(memcached_add(memc_, cast(const char *)key.ptr, key.length,
											   cast(const char *)value.ptr, value.length, cast(time_t)0, cast(uint)0));
	}
	...
	* @return true on success; false otherwise
	*/
	bool addByKey(T = string)(string master_key,
				  string key,
				  T tvalue)
	{
		auto value = to!string(tvalue);
		return memcached_success(memcached_add_by_key(memc_,
													  cast(const char *)master_key.ptr,
													  master_key.length,
													  cast(const char *)key.ptr,
													  key.length,
													  cast(const char *)value.ptr,
													  value.length,
													  cast(time_t)0, cast(uint)0));
	}
	...
	*/
	...
	{
		auto value = to!string(tvalue);
		return memcached_success(memcached_replace(memc_, cast(const char *)key.ptr, key.length,
												   cast(const char *)value.ptr, value.length,
												   cast(time_t)0, cast(uint)0));
	}
	...
	* @return true on success; false otherwise
	*/
	bool replaceByKey(T = string)(string master_key,
					  string key,
					  T tvalue)
	{
		auto value = to!string(tvalue);
	...
	*/
	...
	{
		auto value = to!string(tvalue);
		return memcached_success(memcached_prepend(memc_, cast(const char *)key.ptr, key.length,
												   cast(const char *)value.ptr, value.length, cast(time_t)0, cast(uint)0));
	}
	...
	* @return true on success; false otherwise
	*/
	bool prependByKey(T = string)(string master_key,
					  string key,
					  T tvalue)
	{
		auto value = to!string(tvalue);
	...
	* @return true on success; false otherwise
	*/
	bool appendByKey(T = string)(string master_key,
					 string key,
					 T tvalue)
	{
		auto value = to!string(tvalue);
	...
	*/
	bool cas(T = string)(string key,
			 T tvalue,
			 ulong cas_arg)
	{
		auto value = to!string(tvalue);
		return memcached_success(memcached_cas(memc_, cast(const char *)key.ptr, key.length,
											   cast(const char *)value.ptr, value.length,
	...
	* @param[in] cas_arg "cas" value
	*/
	bool casByKey(T = string)(string master_key,
				  string key,
				  T tvalue,
				  ulong cas_arg)
	{
	...
	* @return true on success; false otherwise
	*/
	bool delByKey(string master_key,
					 string key,
					 int expiration = 0)
	{
